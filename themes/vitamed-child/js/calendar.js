...
function createUseEl(svg, el) {
    ...
    useElement.setAttributeNS("http://www.w3.org/1999/xlink", "href", el);
    return useElement;
}
...
function seasons(next) {
    ...
    for (var i = 0; i < amount; i++) {
        ...
        if (season === "winter") {
            ...
            seasonEl.style.transform = `translate(270px,0) scale(${
                Math.random() + 0.9
            })`;
...
function springAnimation(el) {
    ...
    petals.forEach(
        (petal) => (petal.style.fill = colors[Math.floor(Math.random() * 3) + 0])
    );
...
function sunAnimation(el) {
    ...
    shines.forEach((shine, i) => {
        TweenMax.to(shine, 0.8, {
            morphSVG:
                "M3,8 C7.27916667,12.0320312 17.0177083,20.9453125 21.296875,24.9773437 L3,8 Z",
            repeat: -1,
            delay: i * 0.1,
            ease: Ease.easeOut,
            yoyo: true
